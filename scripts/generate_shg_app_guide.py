...
MD_FILE = os.path.join(DOCS_DIR, "SHG_App_Technical_Guide.md")
HTML_FILE = os.path.join(DOCS_DIR, "SHG_App_Technical_Guide.html")
DOCX_FILE = os.path.join(DOCS_DIR, "SHG_App_Technical_Guide.docx")

def build_markdown_content():
    return """# SHG MOBILE APP (REACT NATIVE / EXPO)
...
> [!IMPORTANT]
> **THE GOLDEN RULE:**  
> **Understand the business flow first $\rightarrow$ understand the code $\rightarrow$ make the smallest safe change $\rightarrow$ test the complete affected flow.**
...
def generate_docs():
    ...
    md_text = build_markdown_content()
    ...
    # Import helper functions from existing generator script
    sys.path.append(os.path.join(r"c:\Users\parth\OneDrive\Desktop\GST-v1\logistic", "scripts"))
    import generate_docs as gd

    # 2. Generate HTML
    gd.HTML_FILE = HTML_FILE
    gd.generate_html(md_text)

    # 3. Generate DOCX
    gd.DOCX_FILE = DOCX_FILE
    gd.generate_docx(md_text)
