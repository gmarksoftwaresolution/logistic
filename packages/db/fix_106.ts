...
async function main() {
  await prisma.pickupOrder.updateMany({
    where: { masterOrder: { orderNumber: 'ORD-2026-106' } },
  ...
  await prisma.$executeRawUnsafe(`
    ...
    WHERE "orderId" = 'ORD-2026-106' AND phase = 'PICKUP';
  `);

  await prisma.masterOrder.updateMany({
    where: { orderNumber: 'ORD-2026-106' },
    data: { status: 'PARCEL_AT_SHG' }
  });

  console.log('Fixed ORD-2026-106 status to PICKED_UP in DB');
}
