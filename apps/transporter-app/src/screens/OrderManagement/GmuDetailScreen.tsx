...
const GmuDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const displayedProducts = gmuProducts.filter((p) => p.type === activeTab);
  ...
  const handleDirectCapture = async (productId: string) => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        completeProduct(productId, 'gmu', capturedUri);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScreenHeader
        title="GMU Tasks"
        subtitle="Gram Marketing Unit Processing"
        showBackButton={true}
      />
...
const styles = StyleSheet.create({
  ...
  primaryActionButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#2ECC71', // Primary action requested green color
  ...
  rejectButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#E74C3C', // Destructive red color requested
    paddingVertical: verticalScale(12),
    borderRadius: scale(10),
  },
  rejectButtonText: {
...
export default GmuDetailScreen;
