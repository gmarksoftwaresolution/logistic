...
import { Check, Trash2, AlertTriangle, X, ChevronLeft, RefreshCw, Info } from 'lucide-react-native';
  ...
  const handleBarcodeScanned = async (event: any) => {
    ...
    if (isQrCodeScan && !data.startsWith('BARCODE:')) {
      try {
        ...
        const res = await api.get(`/qr/info/${encodeURIComponent(cleanId)}`);
        ...
        setInfoParcelData({
          ...
          productName: 'Agri Goods Package',
          quantity: 1,
          weight: '2.5 KG',
          parcelStatus: 'IN_TRANSIT_TO_HUB',
          priority: 'MEDIUM',
        ...
        setTimeout(resetScanLock, 2000);
      }
      return;
    }

    if (!activeSession) {
      setTimeout(resetScanLock, 1000);
      return;
    }
                  ...
                  <View style={styles.orderHeaderRow}>
                    <View>
                      ...
                      <Text style={styles.orderIdValue}>
                        {orderGroup.orderId.startsWith('ORD-') ? orderGroup.orderId : `ORD-${orderGroup.orderId.replace('pickup-', '').replace('drop-', '')}`}
                      </Text>
                    </View>
            ...
            {infoParcelData && (
              <ScrollView style={{ maxHeight: 380 }}>
                ...
                <View style={{ backgroundColor: '#F8FAFC', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: '#E2E8F0', marginBottom: 12 }}>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: '800', color: '#0F172A', marginTop: 2 }}>{infoParcelData.productName || 'Agri Goods'}</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#E2E8F0' }}>
                    <View>
                      ...
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#334155' }}>{infoParcelData.quantity} item ({infoParcelData.weight || infoParcelData.totalWeight || '2.5 KG'})</Text>
                    </View>
                    <View>
