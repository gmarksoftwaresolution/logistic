...
export const HistoryStats = ({ stats }: { stats: StatsType | null }) => {
  ...
  const displayStats = stats || { totalOrders: 0, completedOrders: 0 };

  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20 }} className="mt-3.5 mb-4">
      <View className="bg-white border border-slate-100 rounded-2xl p-3 mr-3 w-[130px] h-[64px] flex-row items-center shadow-sm" style={{ elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3 }}>
        ...
        <View className="flex-1 justify-center">
          <Text className="text-[18px] font-black text-[#111827] leading-none" numberOfLines={1}>{displayStats.totalOrders}</Text>
      ...
      <View className="bg-white border border-slate-100 rounded-2xl p-3 mr-3 w-[130px] h-[64px] flex-row items-center shadow-sm" style={{ elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3 }}>
        ...
        <View className="flex-1 justify-center">
          <Text className="text-[18px] font-black text-[#111827] leading-none" numberOfLines={1}>{displayStats.completedOrders}</Text>
