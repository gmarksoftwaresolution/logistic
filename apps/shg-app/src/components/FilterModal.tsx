...
export const FilterModal: React.FC<Props> = ({ visible, currentFilter, onClose, onApply }) => {
  ...
  const handleDateChange = (event: any, selectedDate?: Date) => {
    ...
    if (selectedDate) {
      if (showPicker === 'start') {
        setTempFilter({ ...tempFilter, startDate: selectedDate });
      } else if (showPicker === 'end') {
        setTempFilter({ ...tempFilter, endDate: selectedDate });
      }
    }
  };
      ...
      <View className="flex-1 bg-black/50 items-center justify-center p-6">
        ...
        <View className="bg-white rounded-[24px] w-full max-w-[400px] p-6 shadow-2xl">
          ...
          <View className="flex-row justify-between items-center mb-6">
            <Text className="text-[20px] font-bold text-textPrimary">Filter Orders</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {filterOptions.map((option) => {
              ...
              return (
                <TouchableOpacity
                  key={option}
                  onPress={() => setTempFilter({ ...tempFilter, type: option })}
            ...
            {tempFilter.type === 'Custom Date Range' && (
              <View className="mt-4 p-4 border border-slate-200 rounded-[16px] bg-slate-50">
                <Text className="text-[13px] font-bold text-textPrimary mb-3">Select Date Range</Text>
                
                <View className="flex-row justify-between items-center">
                  <TouchableOpacity 
                    onPress={() => setShowPicker('start')}
                    className="flex-1 bg-white border border-slate-200 p-3 rounded-[12px] mr-2"
                  >
                    <Text className="text-[11px] text-textSecondary font-bold mb-1">START DATE</Text>
                  ...
                  <TouchableOpacity 
                    ...
                    className="flex-1 bg-white border border-slate-200 p-3 rounded-[12px] ml-2"
                  >
                    <Text className="text-[11px] text-textSecondary font-bold mb-1">END DATE</Text>
                    <Text className="text-[14px] text-textPrimary font-semibold">
          ...
          {showPicker && (
            <DateTimePicker
              value={showPicker === 'start' ? (tempFilter.startDate || new Date()) : (tempFilter.endDate || new Date())}
              mode="date"
              display="default"
          ...
          <View className="flex-row mt-6">
            ...
            <TouchableOpacity
              onPress={handleApply}
              disabled={tempFilter.type === 'Custom Date Range' && (!tempFilter.startDate || !tempFilter.endDate)}
              activeOpacity={0.75}
              className={`flex-1 h-[50px] rounded-[25px] items-center justify-center ml-2 shadow-md ${
                (tempFilter.type === 'Custom Date Range' && (!tempFilter.startDate || !tempFilter.endDate))
                  ? 'bg-slate-300'
                  : 'bg-[#073318]'
              }`}
            >
              <Text className="text-white font-black text-[14px]">Apply</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
