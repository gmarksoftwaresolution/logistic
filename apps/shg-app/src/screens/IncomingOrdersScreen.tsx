...
type Props = CompositeScreenProps<
  NativeStackScreenProps<OrdersStackParamList, 'IncomingOrders'>,
...
const IncomingOrdersScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const { user } = useUser();
  const { incomingOrders, acceptOrder, acceptAllOrders, rejectOrder } = useOrders();
  ...
  const [rescheduledOrders, setRescheduledOrders] = useState<Record<string, { date: string, time: string, reason?: string }>>({});
  ...
  useEffect(() => {
    if (selectedIds.length > 0) {
      ...
      Animated.parallel([
        Animated.timing(animatedY, { toValue: 0, duration: 250, useNativeDriver: true }),
        Animated.timing(animatedOpacity, { toValue: 1, duration: 250, useNativeDriver: true })
      ]).start();
    } else {
      Animated.parallel([
        Animated.timing(animatedY, { toValue: 100, duration: 250, useNativeDriver: true }),
        Animated.timing(animatedOpacity, { toValue: 0, duration: 250, useNativeDriver: true })
      ]).start(() => {
  ...
  const handleAcceptSelected = () => {
    ...
    setModalConfig({
      ...
      onConfirm: () => {
        ...
        setSelectedIds([]);
        Toast.show({ type: 'success', text1: 'Success', text2: 'Orders have been successfully accepted.' });
      }
    });
  };
  ...
  const handleRejectModalSubmit = (order: Order, reason: string) => {
    rejectOrder({ ...order, rejectReason: reason });
    ...
    if (nextIndex < ordersToRejectBatch.length) {
      ...
      import('react-native-toast-message').then(({ default: Toast }) =>
        Toast.show({ type: 'success', text1: 'Done', text2: 'Selected orders have been rejected.' })
      );
    }
  };

  const handleAcceptAll = () => {
    const ordersToAccept = selectedIds.length > 0
      ? incomingOrders.filter(o => selectedIds.includes(o.id))
      : incomingOrders;

    if (ordersToAccept.length === 0) {
      Toast.show({ type: 'error', text1: 'No Orders', text2: 'There are no incoming orders to accept.' });
      return;
    }
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      Toast.show({ type: 'success', text1: 'Refreshed', text2: 'Your order list is up to date.' });
    }, 1500);
  };
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: Colors.background }}>
      <SharedHeader
        title="New Orders"
        subtitle={t('New Orders Sub') || 'Verify & accept incoming requests'}
      ...
      >
        ...
        {incomingOrders.length > 0 && (
          ...
          >
            ...
            <TouchableOpacity
              onPress={() => {
                if (selectedIds.length === 0) {
                  Toast.show({ type: 'error', text1: 'Validation Error', text2: 'Please select at least one order' });
                } else {
                  setRescheduleReasonModalVisible(true);
                }
              }}
              activeOpacity={0.75}
              className="flex-1 h-[50px] bg-white border border-[#CBD5E1] rounded-[25px] flex-row items-center justify-center mr-2 shadow-sm"
              style={{
                shadowColor: '#000',
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.75}
              className="flex-1 h-[50px] bg-[#073318] rounded-[25px] flex-row items-center justify-center ml-2 shadow-md"
              style={{
                shadowColor: '#073318',
          ...
          <View className="px-1" style={{ marginBottom: Spacing.md }}>
            <TouchableOpacity
              onPress={handleSelectAllToggle}
              activeOpacity={0.75}
              className="h-[50px] bg-white border border-[#CBD5E1] rounded-[25px] flex-row items-center justify-center shadow-sm"
              style={{
                shadowColor: '#000',
                shadowOffset: { width: 0, height: 2 },
                shadowOpacity: 0.05,
        ...
        {/* Success Banner */}
        {showSuccessBanner && (
          <View className="p-4 rounded-[16px] bg-[#ECFDF5] border border-[#D1FAE5] flex-row items-center justify-between mb-6 shadow-sm">
            <View className="flex-row items-center flex-1 pr-4">
              <Ionicons name="checkmark-circle" size={24} color="#10B981" />
              <View className="ml-3">
                <Text className="text-[#065F46] font-bold text-[14px]">All orders rescheduled successfully</Text>
                <Text className="text-[#047857] text-[11px] mt-0.5">All orders have been updated with the new date and time.</Text>
              </View>
            </View>
                ...
                <View className="flex-1 pr-2">
                  ...
                  <View className="flex-row items-center gap-1.5 mt-2 flex-wrap">
                    ...
                    <View className="bg-[#EEF2FF] px-2 py-0.5 rounded-[6px] flex-row items-center">
                      ...
                      <Text className="text-[10px] font-black text-[#4F46E5] ml-1">
          ...
          >
            {isRefreshing ? (
              ...
              <>
                ...
                <Text className="text-[#073318] font-bold text-sm ml-2">
                  Refresh Orders
                </Text>
              </>
            )}
      ...
      {shouldRender && (
        ...
        >
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            className="flex-1 flex-row items-center justify-center bg-[#DC2626] py-3.5 rounded-[22px] shadow-sm"
            style={{
              shadowColor: '#DC2626',
              shadowOffset: { width: 0, height: 2 },
              shadowOpacity: 0.15,
          ...
          >
            ...
            <Text className="text-white font-extrabold text-[14px] tracking-wide ml-2">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            className="flex-1 flex-row items-center justify-center bg-[#073318] py-3.5 rounded-[22px] shadow-md"
            style={{
              shadowColor: '#073318',
              shadowOffset: { width: 0, height: 3 },
              shadowOpacity: 0.2,
      ...
      >
        <View className="flex-1 bg-black/50 items-center justify-center p-6">
          ...
          <View className="bg-white rounded-[24px] w-full max-w-[400px] p-6 shadow-2xl">
            {/* Header */}
            <Text className="text-[20px] font-bold text-textPrimary text-center mb-2">
              Reschedule Date & Time
            </Text>
            <Text className="text-textSecondary text-[14px] text-center mb-6">
              Select new date and time for the order
            </Text>

            {/* Calendar Picker Section */}
            <Text className="text-textPrimary font-bold text-[14px] mb-3">Select Date</Text>
            ...
            <View className="border border-slate-100 rounded-2xl p-4 mb-6">
              ...
              <View className="flex-row justify-between items-center mb-4 px-2">
                ...
                <Text className="font-bold text-[15px] text-textPrimary">May 2024</Text>
                <TouchableOpacity className="p-1">
                ...
                {Array.from({ length: 31 }, (_, i) => i + 1).map(day => {
                  ...
                  return (
                    <View key={`may-${day}`} style={{ width: '14.28%', alignItems: 'center' }}>
                      ...
                      >
                        <Text className={`text-[13px] ${isDaySelected ? 'font-semibold text-white' : isPast ? 'font-medium text-gray-300' : 'font-semibold text-textPrimary'
                          }`}>{day}</Text>
                      </TouchableOpacity>
                    </View>
                  );
            ...
            <Text className="text-textPrimary font-bold text-[14px] mb-3">Select Time</Text>
              ...
              {[
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`flex-row items-center px-4 py-2.5 rounded-[12px] border ${isTimeSelected
                  ...
                  >
                    ...
                    <Text className={`text-[13px] font-bold ${isTimeSelected ? 'text-white' : 'text-textSecondary'
                      }`}>{time}</Text>
                  </TouchableOpacity>
                );
              })}
            ...
            <View className="flex-row mt-2">
              ...
              >
                <Text className="text-white font-black text-[14px]">Confirm</Text>
              </TouchableOpacity>
            </View>
      ...
      <Modal
        visible={rescheduleReasonModalVisible}
        transparent={true}
        animationType="fade"
        onRequestClose={() => setRescheduleReasonModalVisible(false)}
      >
        <View className="flex-1 bg-black/50 items-center justify-center p-6">
          <TouchableOpacity
            className="absolute inset-0"
          ...
          <View className="bg-white rounded-[24px] w-full max-w-[400px] p-6 shadow-2xl">
            <Text className="text-[20px] font-bold text-textPrimary text-center mb-6">
            ...
            <ScrollView className="max-h-[300px] mb-4" showsVerticalScrollIndicator={false}>
              {rescheduleReasons.map((reason) => {
                ...
                return (
                  <TouchableOpacity
                    key={reason}
                    onPress={() => setSelectedRescheduleReason(reason)}
                    activeOpacity={0.7}
                    className={`flex-row items-center p-4 mb-3 border rounded-[16px] ${
                      isSelected ? 'border-[#073318] bg-[#F0FDF4]' : 'border-slate-200 bg-white'
                    }`}
                  >
                    <View
                      className={`w-5 h-5 rounded-full border-2 items-center justify-center mr-3 ${
                        isSelected ? 'border-[#073318]' : 'border-slate-300'
                      }`}
                    >
                    ...
                    <Text
                      className={`text-[15px] ${
                        isSelected ? 'font-bold text-[#073318]' : 'font-medium text-textPrimary'
                      }`}
                    >
                      {reason}
                    </Text>
            ...
            {selectedRescheduleReason === 'Other' && (
              <TextInput
                ...
                className="border border-slate-200 rounded-[16px] p-4 text-[15px] text-textPrimary mb-4 bg-slate-50"
                multiline
              />
            )}
            ...
            <View className="flex-row mt-2">
              <TouchableOpacity
                ...
                className="flex-1 h-[50px] bg-white border border-[#CBD5E1] rounded-[25px] items-center justify-center mr-2 shadow-sm"
              >
                <Text className="text-[#4B5563] font-bold text-[14px]">Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => {
                ...
                activeOpacity={0.75}
                className={`flex-1 h-[50px] rounded-[25px] items-center justify-center ml-2 shadow-md ${
                  (!selectedRescheduleReason || (selectedRescheduleReason === 'Other' && !customRescheduleReason.trim()))
                    ? 'bg-slate-300'
                    : 'bg-[#073318]'
                }`}
              >
                <Text className="text-white font-black text-[14px]">Next</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <RejectReasonModal
        ...
        order={ordersToRejectBatch[currentRejectIndex] || null}
        onClose={() => {
          setRejectModalVisible(false);
          setSelectedIds([]);
        }}
