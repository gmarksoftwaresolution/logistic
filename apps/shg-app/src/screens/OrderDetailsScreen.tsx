...
const OrderDetailsScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const { receiveOrder, rejectOrder, deliverOrder } = useOrders();
  ...
  const handleSubmitOrder = () => {
    if (isDeliveryPhase) {
      ...
      deliverOrder({ ...order, scanned: true });
    } else {
      if (!capturedPhotoUri) {
        Alert.alert("Photo Required", "Please capture product photos first.");
        return;
      }
      receiveOrder({ ...order, image: capturedPhotoUri });
    }
    navigation.goBack();
  };
  ...
  const handleRejectModalSubmit = (ord: Order, reason: string) => {
    ...
    rejectOrder({ ...ord, rejectReason: reason });
    navigation.goBack();
  };
      ...
      <ScrollView className="flex-1 px-6 pt-2 pb-10" showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View className="flex-row gap-3">
            <View className="flex-1 bg-white/10 p-3 rounded-[16px] items-center justify-center border border-white/5">
              ...
              <Text className="text-[11px] font-bold text-white/60 mt-1">Items</Text>
            </View>
            <View className="flex-1 bg-white/10 p-3 rounded-[16px] items-center justify-center border border-white/5">
              <Text className="text-[18px] font-black text-white">{order.weight || '10'} kg</Text>
              <Text className="text-[11px] font-bold text-white/60 mt-1">Total Weight</Text>
            </View>
          </View>
        </View>
        ...
        <View className="flex-row justify-between items-center mb-4 mx-1">
          <View className="flex-row items-center">
            ...
            <Text className="text-[16px] font-black text-[#111827] ml-2">Products for Collection</Text>
          </View>
          <View className="flex-row items-center">
        ...
        {/* Task Card Container */}
        <View className="bg-white rounded-[28px] p-2 border border-[#F1F5F9] mb-6" style={{ shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 4 }}>
                ...
                <View className="flex-1 flex-row items-center pr-2">
                  ...
                  <View className="flex-1">
                    <View className="flex-row items-center mb-1">
                      ...
                      <Text className="text-[10px] font-bold text-[#94A3B8]">PENDING</Text>
                    </View>
          ...
          {isDeliveryPhase ? (
            ...
            <TouchableOpacity 
              ...
              className="bg-white border border-[#E2E8F0] rounded-[16px] p-3 mx-2 my-2 flex-row items-center justify-between shadow-sm"
            >
              ...
              <View className="flex-row items-center flex-1 pr-2">
                {capturedPhotoUri ? (
                  <Image 
                    ...
                    className="w-10 h-10 rounded-[8px] mr-3 border border-slate-200" 
                  />
                ) : (
                  <View className="w-10 h-10 rounded-full bg-[#E8F5EC] items-center justify-center mr-3">
          ...
          <TouchableOpacity 
            ...
            className="mx-2 mt-3.5 mb-3 bg-[#FEF2F2] border border-[#FECACA] h-12 rounded-[16px] flex-row items-center justify-center"
          >
            <Ionicons name="close" size={16} color="#DC2626" />
            <Text className="text-[14px] font-bold text-[#DC2626] ml-2">Reject Order</Text>
          </TouchableOpacity>

          {/* Submit Order full-width button */}
          <TouchableOpacity 
            onPress={handleSubmitOrder}
            className="mx-2 mb-2 bg-[#073318] h-12 rounded-[16px] flex-row items-center justify-center shadow-sm"
          >
            <Ionicons name="checkmark-circle-outline" size={16} color="white" />
            <Text className="text-[14px] font-bold text-white ml-2">Submit Order</Text>
          </TouchableOpacity>
        </View>
      ...
      >
        <SafeAreaView className="flex-1 bg-black justify-between p-6">
          <View className="items-center justify-center flex-1 my-4">
            {tempPhotoUri ? (
              <Image 
                ...
                className="w-full h-[75%] rounded-[20px]" 
          ...
          <View className="gap-3 pb-8">
            ...
            <TouchableOpacity 
              onPress={openCamera}
              className="bg-white/20 border border-white/30 h-12 rounded-[16px] flex-row items-center justify-center active:bg-white/30"
            >
              <Ionicons name="camera-outline" size={16} color="white" />
              <Text className="text-[14px] font-bold text-white ml-2">Retake Photo</Text>
            </TouchableOpacity>
            ...
            >
              <Ionicons name="checkmark-circle-outline" size={16} color="white" />
              <Text className="text-[14px] font-bold text-white ml-2">Done</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </Modal>
        ...
        <SafeAreaView className="flex-1 bg-black justify-between p-6">
          ...
          <View className="flex-row items-center justify-between mt-2 px-2">
            <TouchableOpacity 
              ...
              className="w-11 h-11 bg-white/10 rounded-full items-center justify-center border border-white/20"
            >
              <Ionicons name="close" size={24} color="#FFFFFF" />
            </TouchableOpacity>
            <Text className="text-[18px] font-black text-white text-center flex-1 mr-11">
              Verify Products
            </Text>
          </View>
              ...
              <View className="w-[240px] h-[240px] bg-white/5 rounded-[8px] overflow-hidden justify-center items-center">
                {scanningStatus === 'scanning' ? (
                  ...
                  <View className="items-center justify-center">
                    ...
                    <Text className="text-white text-[16px] font-black">Scan Successful</Text>
                  </View>
                )}
              </View>
            </View>
          ...
          <View className="pb-8 items-center justify-center">
            {scanningStatus === 'scanning' ? (
              <View className="flex-row items-center gap-2 bg-white/10 px-4 py-2.5 rounded-full border border-white/10">
                <ActivityIndicator size="small" color="#059669" />
                <Text className="text-[13px] font-bold text-white ml-1">Scanning products...</Text>
              </View>
            ) : (
              <View className="flex-row items-center gap-2 bg-[#E8F5EC] px-4 py-2.5 rounded-full border border-[#D5EFE0]">
                <Ionicons name="checkmark-circle" size={16} color="#073318" />
                <Text className="text-[13px] font-black text-[#073318] ml-1">Verification Complete</Text>
              </View>
            )}
          </View>
        </SafeAreaView>
