...
export default function AddressScreen({ navigation }: Props) {
  ...
  const hasChanges = () => {
    return (
      ...
      formData.stateName !== user.stateName ||
      formData.district !== user.district ||
      formData.taluka !== user.taluka ||
      formData.village !== user.village ||
      formData.homeAddress !== user.homeAddress
    );
  };
    ...
    <View className="w-full mb-6">
      ...
      <View className="flex-row items-center bg-white py-3 px-4 rounded-xl border border-gray-200 shadow-sm">
        <TextInput 
          ...
          className="flex-1 text-textPrimary font-semibold text-base"
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
        <View className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-50 mb-10">
          <InputField label={t('pincode')} value={formData.pincode} onChangeText={(val: string) => setFormData({...formData, pincode: val})} placeholder="Enter pincode" keyboardType="numeric" />
          <InputField label={t('state')} value={formData.stateName} onChangeText={(val: string) => setFormData({...formData, stateName: val})} placeholder="Enter state" />
          <InputField label={t('district')} value={formData.district} onChangeText={(val: string) => setFormData({...formData, district: val})} placeholder="Enter district" />
          <InputField label={t('taluka')} value={formData.taluka} onChangeText={(val: string) => setFormData({...formData, taluka: val})} placeholder="Enter taluka" />
          <InputField label={t('village')} value={formData.village} onChangeText={(val: string) => setFormData({...formData, village: val})} placeholder="Enter village" />
          <InputField label={t('home_address')} value={formData.homeAddress} onChangeText={(val: string) => setFormData({...formData, homeAddress: val})} placeholder="Enter home address" />
      ...
      <Modal visible={showSuccess} transparent={true} animationType="fade">
        <View className="flex-1 justify-center items-center bg-black/60 px-10">
          <View className="bg-white p-10 rounded-[40px] items-center w-full shadow-2xl">
            ...
            <Text className="text-textSecondary text-center text-sm">Address details updated successfully.</Text>
          </View>
        </View>
      </Modal>
    </SafeAreaView>
